...
# over AWS IoT. Reverse-engineering the Govee Android app (docs/
# govee-protocol-reference.md, H7150/H7152 entry) found that account-topic
# "status" pushes are overwhelmingly *replies* to an explicit per-device
# status query the app's device-list screen sends every ~30-60s while it is
# on screen (`AbsOnlyIotModel.checkIotOnline()`/`Iot.A()`) — devices are not
# reliably autonomous pushers. Without ever asking, this integration could go
# quiet the moment the Govee app was closed. Configurable so accounts with
# many devices can back off from the app's own cadence.
