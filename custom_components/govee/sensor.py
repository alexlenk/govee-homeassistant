...
async def async_setup_entry(
    ...
    for device in coordinator.devices.values():
        ...
        # healthy 60s Cloud API poll keeps it looking fresh even when MQTT
        # specifically has been silent for hours — misleading for diagnosing
        # MQTT-only fields like the H7152's temperature/humidity/pump-fault,
        # which have no poll fallback at all. This is the MQTT-only signal.
        if coordinator.mqtt_client is not None:
...
class GoveeMqttLastReceivedPerDeviceSensor(GoveeEntity, SensorEntity):
    ...
    keeps it looking fresh even when MQTT specifically has gone quiet for
    hours — misleading for diagnosing MQTT-only fields like the H7152's
    temperature/humidity/pump-fault, which have no capability or poll
    fallback at all. This sensor isolates the MQTT-only signal instead.
    """
