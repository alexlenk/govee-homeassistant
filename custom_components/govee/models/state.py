...
class GoveeDeviceState:
    ...
    def update_temperature_from_frames(self, frames: Iterable[bytes]) -> bool:
        ...
        shows a live reading) — the app's temperature readout travels over
        this same AWS IoT push rather than any REST field.
        ...
        diagnostics) capture pairs, spanning 20.9-22.4°C, residuals 2-4 raw
        counts (~0.05-0.1°C) — small enough to be capture-timing/display-
        rounding noise rather than a bad fit. NOT validated outside that
        narrow range and the exact slope/intercept may drift as more
        real-world data comes in; compare against the app's own temperature
        history to refine.

        Byte offset 5 of the same frame is a separate, still-unidentified
        value — confirmed (via a live capture comparing an independently
        verified motor-on moment against two motor-off moments) NOT to be
        pump-motor state, and does not correlate with humidity in samples
        collected so far. Left undecoded rather than guessed.
        ...
        """
        for raw in frames:
            if len(raw) >= 6 and raw[0] == 0xAA and raw[1] == 0x10 and raw[2] == 0x81 and raw[3] == 0x03:
