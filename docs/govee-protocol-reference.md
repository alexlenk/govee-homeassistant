...
- The `range`/`humidity` `state.value` comes back as an **empty string `""`** in the poll, and there is **no `sensorTemperature` / `sensorHumidity` capability at all** on these dehumidifiers — confirmed in govee2mqtt #413 too. The Govee app shows live temp/humidity/dew-point/pressure regardless (and remotely, off local network — ruling out a direct BLE-only read); the values almost certainly arrive over the AWS IoT MQTT push's `op.command` BLE-format frames the same way `pumpAbnormal` below does, but the byte layout for these four readings is not yet decoded (two paired app-screenshot + diagnostics captures on an H7152 showed one candidate frame moving too little between samples with near-identical readings to confirm a mapping — needs a capture spanning a bigger swing in conditions).
- **H7152 pump-fault ("Pump Abnormality" in the app) — reverse-engineered from a live clogged-drain capture, issue #114 follow-up.** A separate condition from `waterFullEvent`; the app raises it independently. It is **not** a capability (absent from the discovered capabilities list even while the fault is active), **not** pushed over the OpenAPI event channel, and **not** in the flat MQTT `state` keys — all three confirmed empty across two captures taken during an active fault, two minutes apart. The only signal is **byte offset 11 of an `aa 17` status frame** in the AWS IoT push's `op.command` list:
  ```
  normal:          aa 17 00 00 00 00 00 00 00 00 00 [00] 00 00 00 00 00 00 00 bd
  pump abnormal:   aa 17 00 00 00 00 00 00 00 00 00 [01] 00 00 00 00 00 00 00 bc
  ```
  Confirmed clean across three real captures (before / during ×2 / after an actual fault), checksum consistent with the single-bit flip each time. It is a **live/level flag**, not edge-latched like `waterFullEvent` — it cleared on its own once the app's alert cleared, no "Clear Alert" button needed. SKU-locked to **H7152** only (`PUMP_DEHUMIDIFIER_SKUS`) — H7150/H7151 are non-pump variants with no confirmed frame layout.
