...
- **H7152 `op.command` frame map — reverse-engineered on a real device, 2026-09-10/11, issue #114 follow-up.** A normal `status` push carries ~15 fixed-position frames; pump fault, mode, and both temperature and humidity are all decoded. All findings below are SKU-locked to **H7152** (`PUMP_DEHUMIDIFIER_SKUS`) — H7150/H7151 are non-pump variants with no confirmed frame layout of their own.

  - **Pump fault (`aa 17` frame), byte offset 12** — `0x00` normal, `0x01` while "Pump Abnormality" is showing in the app:
    ```
    normal:          aa 17 00 00 00 00 00 00 00 00 00 00 [00] 00 00 00 00 00 00 bd
  ...
  - **Temperature + live humidity (`aa 10 81 03` frame), byte offsets 3–5 — RESOLVED 2026-09-11, supersedes the "byte 5 is noise, humidity absent" conclusion below.** This is the app's own BLE status frame — opcode `0x10` (`BleProtocolConstants.P()`) — and bytes 3–5 are not three independent fields but one **big-endian packed integer**, not two independent single-byte channels:
    ```
    raw = (frame[3] << 16) | (frame[4] << 8) | frame[5]
    temperature_c = (raw // 1000) / 10.0
    humidity_pct  = (raw %  1000) / 10.0
    ```
    Decoded straight from the official Govee Android app (v7.6.21, decompiled): `com.govee.base_h71xx.sku_base.adjust.component.bean.ThermometerInfo.Companion` (functions `a()`/`b()`/`c()`) is the shared decoder every WiFi combo sensor SKU uses, invoked for this exact opcode by `com.govee.h7160.iot.CmdStatusParseV1` — the H7160 humidifier ships the identical `aa 10` frame and calls the identical shared class, confirming this is a cross-SKU format, not H7152-specific. Byte 3 is `0x03` across the whole room-temperature range captured so far, which is why earlier passes folded it into the frame's fixed "identifier" prefix instead of treating it as data.

    Confirmed against 9 real capture pairs total (the 5 paired app-screenshot/diagnostics captures below, spanning 20.9–22.4°C, plus 4 independent raw-frame/`sensor_temperature_c` pairs) with **zero residual error** — an exact decode, not a fit. The earlier per-byte analysis (below) never found this because it treated byte 4 and byte 5 as separate channels and looked for humidity to correlate with byte 5 in isolation; byte 5 is actually the low byte of the *same* packed integer as byte 4, so its apparent "jitter" was really the low-order digits of the combined temp+humidity value moving independently of the high-order (temperature) digits. **Humidity confirmed correct 2026-09-11**, cross-checked live against the app's own displayed humidity %.

    **Validated against 4 further live H7152 pushes** (account topic `GA/…`, captured 2026-09-11) — every decoded temperature reproduces the earlier fit exactly, and humidity falls out of the same bytes for free:

    | `frame[3:6]` | `raw` | temperature | humidity |
    |---|---|---|---|
    ...
    **Known limitation, largely resolved 2026-09-11 by an active MQTT status re-query (see §4.6/§9.5, `GoveeAwsIotClient.async_publish_status_query`):** this frame (and the `aa 17` pump-fault frame) is a *reply*, not an autonomous push — reverse-engineering the Android app found its device-list screen re-queries every device's status roughly every 30-60s while on screen (`AbsOnlyIotModel.checkIotOnline()`/`Iot.A()`/`Cmd4Status`), and most account-topic "status" traffic observed here was a reply to that exact query. Without ever asking, temperature/humidity/pump-fault all went stale overnight with the Govee app closed — none of the three has a capability or Developer-poll fallback, so a stale value sat there indefinitely rather than clearing. This integration now periodically publishes the same query itself (`mqtt_status_interval` option, default 5 min) so these fields keep updating without the app open; see the option's own docstring for the request/response shape. This mirrors an identical, independently-documented limitation for the H5192 probe thermometer's six-value status frame (`GoveeCoordinator._poll_probe_thermometers`'s docstring) — that frame's active-poll equivalent (`0x24` read) was already implemented before this one, for the same reason.
    - **Superseded analysis, kept for the record:** byte offset 4 alone was originally fit as `°C ≈ 0.025327 × byte + 19.6129` (least-squares over the same 5 captures, residuals 2–4 raw counts) and byte offset 5 was analyzed in isolation and ruled out as humidity/dew-point/pressure — see the exhaustive per-byte diff below. Both conclusions were artifacts of not recognizing bytes 3–5 as one packed value.

    This also explains the earlier byte-5-only analysis below rather than contradicting it: treating byte 5 in isolation, its contribution to the packed value's modulo is not linear in byte 5 alone (it also depends on byte 4 via the shared modulo), so two pushes 3 seconds apart with byte 4 unchanged can show a large byte-5 swing that is real humidity movement, not sensor noise — a plausible reading in a device that is actively pulling moisture out of the air it's measuring. The historical analysis is left in place below for the record; treat its "not humidity" verdict as superseded by the above, not as a second opinion to weigh against it.
    ...
    **`ptReal` while the app was actively open — tried, came back empty.** Every `status` push's `op.command` array contains exactly the same 15 frames every time, no more, no less; the one `ptReal` message seen has a literal `"op": {}` and `"state": {"result": 1}` — this part of the finding stands: `ptReal` genuinely returns nothing extra. What was wrong was the conclusion drawn from it (that humidity must therefore be entirely absent) — it was already present in the `status` push's own `aa 10` frame, just not decoded yet.
    ...
    If Govee's backend also publishes device state to a *different* topic — most plausibly a standard **AWS IoT device shadow** (`$aws/things/<thing-name>/shadow/...`), a common pattern for "current full device state" that's architecturally distinct from the event-stream topic this integration listens to — some other field could still live there. That remains unconfirmed either way and is now irrelevant to humidity, which did not require it.
      ...
      **Wildcard-subscribe diagnostic — implemented, then DISABLED; the disconnect theory is now confirmed against AWS's own documentation, not just inferred.** `GoveeAwsIotClient` could optionally subscribe to `<account_topic>/#` (e.g. `GA/<accountId>/#`) right after the account topic was confirmed up (`_try_wildcard_subscribe`), purely to see what AWS IoT's per-certificate SUBACK said — deliberately scoped under the account's own topic rather than a bare `#`. `_start_mqtt` originally opted an account in whenever an H7152 was registered; on the one real-world test, the H7152 debug log went silent and HA logged `AWS IoT connection dropped early (MqttError: Disconnected during message iteration)` shortly after upgrading to the build that enabled it.
      ...
      AWS's own IoT Core documentation confirms this is exactly the documented, designed behavior, not a fluke: **"When a client performs an action that is not allowed by its policy, the client will be disconnected by the server."** Unauthorized `SUBSCRIBE` is such an action. That reconciles the mechanism precisely: MQTT 3.1.1 still requires a per-topic-filter SUBACK for every SUBSCRIBE, so `_try_wildcard_subscribe`'s own `client.subscribe(...)` call may well have returned cleanly (granted or a `0x80` refusal — the confirming INFO log wasn't visible at HA's default WARNING-only level for custom integrations) — but AWS IoT's policy enforcement then dropped the *entire session* as a separate, asynchronous consequence, which is why the failure surfaced downstream in the `async for message in client.messages` loop rather than as an exception from the subscribe call itself. No amount of try/except scoped around just the subscribe call could have caught that — the "best-effort, never affects the primary session" design assumption was structurally unable to hold, given how AWS IoT actually enforces policy.

      Given that, the risk to a live MQTT connection outweighed this diagnostic's marginal value, so it was disabled (`attempt_wildcard_subscribe=False` hardcoded) and, once the H7152 humidity question above was independently resolved via the packed-value decode (making this probe's own motivation moot), removed entirely — `_try_wildcard_subscribe`, the `attempt_wildcard_subscribe` parameter, and the `RawMessageCallback`/`AnyMessageCallback`/`_log_h7152_debug_*` scaffolding it was paired with no longer exist in the codebase (2026-09-11). Separately, AWS explicitly recommends *against* wildcard subscriptions to shadow topics specifically (`$aws/things/thingName/shadow/#`), even for topics a well-behaved client is authorized to read, since new shadow sub-topics can be added over time — reinforcing that a wildcard probe was never going to be a clean, low-risk way to explore AWS IoT topics in general. Device shadow topics live under the entirely separate `$aws/things/<thingName>/shadow/...` namespace (confirmed via AWS docs), which this account-topic-scoped probe could never have reached even had it been granted — so even a "successful" run of this diagnostic would not have found a shadow topic; it only ever tested for siblings/children of the account's own topic, which is now a closed question either way.
  - **Target humidity + Comfort mode (`aa 05 03` frame)**. Bytes 5–6 are the live target-humidity setpoint in centi-percent big-endian (`0x157c` = 5500 = 55.00%), confirmed by watching it track an in-app slider change in near real-time — **faster than and independent of** the REST-polled `range/humidity` capability value, which can lag behind it. Byte offset 4 is the **Comfort mode toggle**: `0x00` = off, `0x01` = on — confirmed by an hour-plus steady `1`, followed immediately (within the stated margin of a rough time estimate) by a 40+ minute unbroken `0` exactly matching a real in-app Comfort-off action. (Brief single-sample `0` blips also appeared during active slider drags — likely a transient write-in-progress pulse, not a real mode change; only a flag value **held stable across multiple pushes** should be trusted.) Not yet explained: the reported target value itself also differed between the two Comfort states (50% while on, 55% while off) — real, but cause unconfirmed; a test that toggles Comfort *without* touching the slider would settle it.
  - **Tank-vs-hose/pump mode (`aa 19` frame), byte offset 9 — confirmed 2026-09-11.** `0x01` while the drain hose reads as connected (app's "Pump Mode"), `0x00` once it doesn't (app's "Water Tank Mode", draining into the bucket instead). Confirmed by repeatedly pressing and holding the device's own hose-connection button (~5s per hold, 5 cycles) and watching the app's Mode label toggle in exact lockstep with this byte on every transition. Byte offset 7 of the same frame also moves but is deliberately left undecoded — first suspected to be pump-motor state, but a frame-by-frame diff against independently confirmed motor-on/motor-off moments showed the entire `aa 19` frame identical across all three, so the pump's running state isn't transmitted in this push at all.
  - **Architecture note:** the `multiSync` MQTT message type mirrors several of these same status-push sub-frames, with a different envelope per frame — the `aa 05 03` setpoint frame reappears as `3a 05 03 ...` (same payload, `aa`→`3a` header swap), and the `aa 10 81 03` temperature frame reappears wrapped as `aa 01 01 00 81 03 ...`. `multiSync`/`ptReal` pushes never reach the coordinator's normal state-update path at all (see `api/mqtt.py`'s early-return branches) — they had to be captured separately. A second gap existed one level below that: `_handle_message`'s own `"msg"`-wrapped unwrap-or-return branch silently drops any payload lacking a top-level `"device"`+`"state"` pair *before* the multiSync/ptReal capture hook's call site is even reached — confirmed live, since the one `ptReal` ack captured turned out to be exactly that shape. `GoveeAwsIotClient`'s `on_any_message` hook (wired to `GoveeCoordinator._on_mqtt_any_message` / `_log_h7152_debug_raw_payload`) fires before any parsing at all, logging the untouched wire JSON, so nothing can hide in an unrecognised envelope shape.
  - **Cross-checked against other Govee reverse-engineering projects — conclusively closes the "another topic" question.** [`wez/govee2mqtt`](https://github.com/wez/govee2mqtt) (Rust, actively maintained) has the *exact* idea this integration also tried — subscribing to each device's own `GD/`-style topic (fetched the same way this integration's `_device_topics` are, from `deviceExt.deviceSettings.topic`) alongside the account topic — already implemented and explicitly disabled (`src/service/iot.rs`, commit `6f5a844`, 2026-04-08):
    ```rust
  ...
  - **govee2mqtt's own BLE frame decoder confirms the same absence, for every SKU it supports, not just the H7152 — this conclusion applies to `PacketCodec`-style single-byte/bitfield decoding, and is now known NOT to rule out a multi-byte packed value like the one found above.** Its `PacketCodec` registry (`src/ble.rs`) only ever decodes a **target/setpoint** humidity frame — `0xaa 0x05 0x03` for the `H7160` humidifier, the *same opcode* this investigation independently found for the H7152's target-humidity setpoint — never a live/current-humidity reading. No `GoveeBlePacket` variant for any SKU decodes a current humidity value from an AWS IoT frame anywhere in that project. Their own `sensorHumidity` sensor (`src/hass_mqtt/sensor.rs`) reads the REST/Platform API capability value instead — the same channel this integration's docs already note returns an empty string for the H7150/H7151/H7152 family. **Correction (2026-09-11):** the exhaustive per-byte analysis above and the cross-project comparison here both missed the packed-value encoding documented above, because both were looking for humidity as an independent byte/bitfield rather than sharing a multi-byte arithmetic value with the already-identified temperature byte. Live humidity for the H7152 IS transmitted over AWS IoT — just packed alongside temperature in the same `aa 10 81 03` frame's bytes 3–5, not as a separate channel — using the same shared `ThermometerInfo` decode the official Govee Android app applies on every WiFi combo-sensor SKU it supports, H7160 included, per the decompiled app source referenced above.
  ...
  - All of the above was captured via a **TEMPORARY debug log** (`govee_h7152_debug.jsonl` in the HA config dir, via `GoveeCoordinator._append_h7152_debug_line`) added specifically for this investigation, and removed 2026-09-11 once temperature, pump-fault, mode, and (via the packed-value decode) humidity were all identified — the raw capture is no longer needed day-to-day, and this document plus `tests/test_pump_abnormal.py` preserve the evidence trail. The staleness this investigation also surfaced (see "Known limitation" above) is addressed separately by the periodic MQTT status re-query (`async_publish_status_query`, §4.6/§9.5), not by this debug log.
