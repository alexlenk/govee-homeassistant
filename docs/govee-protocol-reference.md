...
- **H7152 `op.command` frame map — reverse-engineered on a real device, 2026-09-10, issue #114 follow-up.** A normal `status` push carries ~15 fixed-position frames; three are decoded, one confirmed still-unidentified. All findings below are SKU-locked to **H7152** (`PUMP_DEHUMIDIFIER_SKUS`) — H7150/H7151 are non-pump variants with no confirmed frame layout of their own.
  ...
  - **Tank-vs-hose/pump mode is not yet identified at all.** The H7152 only engages its pump when a drain hose is physically connected (hose-detection switch per the user manual); with the water tank/bucket instead, the pump never activates. No capture so far has been taken with the hose disconnected, so there's no data point yet to check against any of the above.
