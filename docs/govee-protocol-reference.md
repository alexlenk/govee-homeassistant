...
- **H7152 `op.command` frame map — reverse-engineered on a real device, 2026-09-10/11, issue #114 follow-up.** A normal `status` push carries ~15 fixed-position frames; pump fault, mode, and now both temperature and humidity are all decoded. All findings below are SKU-locked to **H7152** (`PUMP_DEHUMIDIFIER_SKUS`) — H7150/H7151 are non-pump variants with no confirmed frame layout of their own.
  ...
  - **Temperature + humidity (`aa 10 81 03` frame), bytes 3–5 — SUPERSEDES the byte-4-only linear fit and the "byte 5 is not humidity" conclusion below.** This is the app's own BLE status frame — opcode `0x10` (`BleProtocolConstants.P()`), decoded app-side by `CmdStatusParseV1` into a `ThermometerInfo` — and bytes 3–5 are a single big-endian 3-byte packed value, not two independent single-byte channels:
    ```
    ...
    **Known limitation, confirmed live 2026-09-11: this frame (and the `aa 17` pump-fault frame) appears to arrive only while a Govee app session is/was recently active against the device, not on any independent timer.** Left overnight with the app closed, temperature/humidity/pump-fault all went stale — no update at all, and since none of the three has any capability or Developer-poll fallback, a stale value just sits there indefinitely rather than clearing to unknown. This mirrors an identical, previously-documented limitation for the H5192 probe thermometer's six-value status frame (`GoveeCoordinator._poll_probe_thermometers`'s docstring: "arrives unprompted on power-on and when an app session opens; no request was found that triggers it") — no AWS IoT command has been found that provokes any of these three H7152 frames on demand. Until/unless one is found, treat these three as best-effort, app-session-gated readings, not continuously live ones.
  ...
  - All of the above was captured via a **TEMPORARY debug log** (`govee_h7152_debug.jsonl` in the HA config dir, via `GoveeCoordinator._append_h7152_debug_line`) added specifically for this investigation, and removed 2026-09-11 once temperature, pump-fault, mode, and (via the packed-value decode) humidity were all identified — the raw capture is no longer needed day-to-day, and this document plus `tests/test_pump_abnormal.py` preserve the evidence trail.
